...
export function VoiceIndicator({ voice }: { voice: VoiceStatus }) {
  return voice.active ? (
    <span className="inline-flex items-center gap-2 rounded-full bg-risk-high/[.13] px-2.5 py-[5px] text-[12px] font-semibold text-risk-high">
      <span className="h-[7px] w-[7px] animate-pulse rounded-full bg-risk-high shadow-[0_0_8px_#ef4444]" />
      {voice.label}
    </span>
  ) : (
    <span className="inline-flex items-center gap-2 rounded-full border border-line bg-elevated px-2.5 py-[5px] text-[12px] font-semibold text-muted">
