 ...
 *
 * A CLIENT component, deliberately: the active locale lives in localStorage and
 * is read on mount by LocaleProvider (components/i18n/locale-provider.tsx), so
 * the server cannot know it. While this markup lived directly in the server
 * `page.tsx` none of its copy could be translated — the screen rendered its
 * headline, pillar blurbs and trust chips in English next to the (client, and
 * therefore translated) sign-in form. This is the first screen anyone sees.
 *
 * `page.tsx` stays a server component so it can still export `metadata`.
 */
...
/** The four pillars. `k` spells I-T-T-U — the product's own acronym — and the
 *  NAMES stay untranslated for the same reason they do in the sidebar and the
 *  guide: they are product terms, not prose. Only the blurbs are translated. */
const PILLARS = [
  { k: "I", name: "Infiltrate", key: "infiltrate" },
  { k: "T", name: "Trace", key: "trace" },
  { k: "T", name: "Takedown", key: "takedown" },
  { k: "U", name: "Uncover", key: "uncover" },
] as const;

const TRUST = ["encrypted", "isolated", "audit"] as const;
        ...
        <aside className="relative hidden w-[50%] flex-col justify-between p-10 lg:flex xl:p-12">
          <div className="login-rise relative flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[12px] text-[#999]">{t("brandTagline")}</div>
            </div>
          </div>
              ...
              {PILLARS.map((p, i) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[12px] leading-snug text-[#999]">
                      {t(`pillars.${p.key}`)}
                    </div>
                  </div>
                </li>
        ...
        <main className="relative flex flex-1 items-center justify-center px-6 py-10">
          <div className="w-full max-w-[380px]">
            ...
            >
              ...
              <div>
                <h1 className="text-[17px] font-semibold tracking-tight">{t("signInTitle")}</h1>
                <p className="text-[12px] text-[#999]">{t("signInSub")}</p>
              </div>
            </div>
              ...
              {TRUST.map((k) => (
                ...
                >
                  <div className="flex items-center justify-center gap-1 text-[12px] font-semibold text-white/80">
                    <span className="h-1 w-1 flex-none rounded-full" style={{ background: "#0099ff" }} aria-hidden />
                    {t(`trust.${k}Label`)}
                  </div>
                  <div className="mt-0.5 text-[12px] leading-tight text-[#999]">
                    {t(`trust.${k}Sub`)}
                  </div>
                </div>
              ))}
