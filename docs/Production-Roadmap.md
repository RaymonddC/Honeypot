...
- **Demo nuance:** enabling `takedown=live` makes the Investigation screen query *real* chains — the seeded honeypot fixture wallet (`TLaJLQ1Aiqu8YpZLNG37vDCrFxiFL3JWmn`) won't resolve on-chain, so the scripted honeypot→investigation demo narrative needs POC fixtures. Keep `takedown=poc` for the seeded story; use `live` to investigate real wallets/cases (both coexist via MODE).
