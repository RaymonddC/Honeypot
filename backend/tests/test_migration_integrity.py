...
def test_models_match_migrations(migrated_pg) -> None:
    ...
    Autogenerate is not omniscient (it does not diff CHECK constraints, RLS
    policies, or server-side functions), so a green result here means "no
    detectable table/column/index/constraint drift", not "the migration is
    complete". It still catches the common case: a column added to a model.
    """
    import alembic.command
