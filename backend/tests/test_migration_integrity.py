...
The two directions of drift need different guards, and both are covered here:

- **DB behind the code** — ``test_single_migration_head`` (DB-less, always runs)
  and ``test_full_chain_applies_and_has_case_stage`` (runs the WHOLE Alembic
  chain against an ephemeral in-process Postgres — pgserver, no Docker — and
  asserts it reaches head with the schema the app needs).
- **Code ahead of the migrations** — ``test_models_match_migrations`` runs
  ``alembic check``, catching the opposite mistake: someone edits an ORM model
  and never writes the migration. That is exactly how the ``stage`` outage was
  authored in the first place; nothing failed until a deploy hit the gap.

The pgserver-backed tests skip cleanly when pgserver isn't installed, same as
the sibling ``*_pg.py`` suites.
"""
...
def test_models_match_migrations(migrated_pg) -> None:
    """``alembic check``: every ORM model change must have a migration behind it.

    The sibling tests catch a database that is BEHIND the code. This one catches
    the reverse — a model edited with no migration written — which is how that
    gap gets authored in the first place. It runs against the ephemeral cluster
    the fixture already migrated to head, so CI needs no external database.

    Scope: tables, columns, indexes, FK and unique constraints. CHECK
    constraints are excluded on purpose (see ``include_object`` in
    ``migrations/env.py``) — this project authors them in raw SQL in the
    migrations and never restates them on the models, and a newer Alembic began
    reporting all ~48 as phantom "removed" diffs, making the guard depend on
    whichever version CI resolved. RLS policies and server-side functions are
    likewise beyond autogenerate's reach.

    So green here means "no detectable table/column/index/FK/unique drift", NOT
    "the migration is complete" — those other objects are reviewed by reading
    the migration. It still catches the common case: a column added to a model.
    """
    import alembic.command
    ...
    try:
        ...
        pytest.fail(
            "ORM models and migrations disagree — a model was changed without a "
            "migration.\n\n"
            f"{diff}\n\n"
            "Fix: generate one, review it (autogenerate guesses names and misses "
            "CHECK constraints / RLS), then re-run:\n"
            "    cd backend && alembic revision --autogenerate -m '<what changed>'\n"
            "If instead the DATABASE is the odd one out, do NOT hand-edit models to "
            "silence this — work out which side is authoritative first."
        )
